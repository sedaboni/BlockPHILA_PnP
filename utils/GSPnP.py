...
class GSPnP(Denoiser):
    ...
    def potential(
        ...
        with torch.no_grad():
            ...
            out = 0.5 * self.alpha * torch.norm((x - N).view(x.shape[0], -1), p=2, dim=-1) ** 2
        return out

    def potential_grad(
        self, x: Tensor, sigma: Union[float, torch.Tensor], Ui=None, *args, **kwargs
    ) -> Tensor:
        r"""
...
def GSDRUNet(
    ...
    if den_type=='dncnn':
        denoiser = deepinv.models.DnCNN(device=device)
    elif den_type=='swinir':
        denoiser = deepinv.models.SwinIR(device=device)
    elif den_type=='scunet':
        denoiser = deepinv.models.SCUNet(device=device)
    elif den_type=='restormer':
        denoiser = deepinv.models.Restormer(device=device)
    elif den_type=='drunet_vanilla':
        denoiser = DRUNet(
        ...
        raise ValueError(f"unknown den_type {den_type!r}")
