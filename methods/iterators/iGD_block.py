...
class iGD_block(BlockMethod):
    ...
    def iteration(self, x, y, physics, x_true, filename):
        ...
        for itr in range(1,self.maxitr+1):
            ...
            yk = x - self.tau[self.block_counter]*Dg + self.beta_inertial*diff
            x_guess = yk
            dk = yk - x
            norm_dk = dk.norm()**2
            ...
            Delta = ((Dg-(self.beta_inertial/self.tau[self.block_counter])*diff)*dk).sum() + (dk.norm()**2)/(2*self.tau[self.block_counter])
