...
class PHILA(Method):
    ...
    def iteration(self, x, y, physics, x_true, filename):
        ...
        for itr in range(1,self.maxitr+1):
            ...
            z = x - (self.tau*self.lamb)*Dg + self.beta_inertial*(diff)
            yk = self.data_fidelity.prox(z,y,physics, gamma=self.tau)
            dk = yk - x
            norm_dk = dk.norm()**2
            ...
            Delta = ((self.lamb*Dg-(self.beta_inertial/self.tau)*diff)*dk).sum() + (dk.norm()**2)/(2*self.tau) - f_x + f_y.clone()
