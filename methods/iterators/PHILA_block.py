...
class PHILA_block(BlockMethod):
    ...
    def iteration(self, x, y, physics, x_true, filename):
        ...
        for itr in range(1,self.maxitr+1):
            ...
            z = x - (self.tau[self.block_counter]*self.lamb)*Dg + self.beta_inertial*diff
            ...
            Delta = ((self.lamb*Dg-(self.beta_inertial/self.tau[self.block_counter])*diff)*dk).sum() + (dk.norm()**2)/(2*self.tau[self.block_counter]) - f_x + f_y
