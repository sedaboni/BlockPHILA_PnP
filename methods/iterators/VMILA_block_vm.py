...
class VMILA_block_vm(BlockMethod):
    ...
    def iteration(self, x, y, physics, x_true, filename):
        ...
        for itr in range(1,self.maxitr+1):
            ...
            z = x - (self.tau[self.block_counter]*self.lamb)*self.Ds*Dg
            ...
            Delta = self.lamb*(Dg*dk).sum() + ((torch.pow(dk,2)/self.Ds).sum())/(2*self.tau[self.block_counter]) - f_x + f_y
