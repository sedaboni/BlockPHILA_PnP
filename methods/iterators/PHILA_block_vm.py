...
class PHILA_block_vm(BlockMethod):
    ...
    def iteration(self, x, y, physics, x_true, filename):
        ...
        for itr in range(1,self.maxitr+1):
            ...
            z = x - (self.tau[self.block_counter]*self.lamb)*self.Ds*Dg + self.beta_inertial*diff

            yk = self.data_fidelity.prox_scaled(z,y,physics, xbar=x, Uitilde=self.Uitilde, gamma=self.tau[self.block_counter], D_scaling=1/self.Ds)
            dk = yk - x
            norm_dk = dk.norm()**2
            ...
            Delta = ((self.lamb*Dg-(self.beta_inertial/self.tau[self.block_counter])*diff/self.Ds)*dk).sum() + ((torch.pow(dk,2)/self.Ds).sum())/(2*self.tau[self.block_counter]) - f_x + f_y
    ...
    def update_beta(self, itr):
        i = (itr-1)//self.nblocks
        self.beta_inertial = (i)/(i + 3)